      ...
      <template #header>
        <q-item class='q-py-md'>
          <q-item-section avatar>
            <q-icon name='sym_o_contact_phone' color='primary' />
          </q-item-section>
          <q-item-section>
...
export default {
  ...
  setup(props, context) {
    ...
    const { t: $t } = useI18n()
    const { emit } = context
    const { contactForm } = toRefs(props)
    ...
    const valueFieldIcon = computed(() => {
      switch (form.type) {
        case ContactMethodsEnum.PHONE: return 'sym_o_call'
        case ContactMethodsEnum.EMAIL: return 'sym_o_email'
        case ContactMethodsEnum.FAX: return 'sym_o_fax'
        case ContactMethodsEnum.WEBSITE: return 'sym_o_travel_explore'
